...
ORDER BY lecture_number;`, in.Id)
	...
	for rows.Next() {
		...
		t := time.Unix(int64(*status.Encoding.Metadata.Duration.Get()), 0).UTC()
		videoDocument.Duration = &learning.Duration{
			Hours:       int32(t.Hour()),
			Minutes:     int32(t.Minute()),
			Seconds:     int32(t.Second()),
			Nanoseconds: int32(t.Nanosecond()),
...
func (s *Server) GetPromoDocumentsByLesson(lessonID string) (*learning.Documents, error) {
...
FROM documents 
WHERE lesson_id = $1
ORDER BY lecture_number;`, lessonID)
	if err != nil {
		s.Logger.Error(err.Error())
		if errors.Is(err, sql.ErrNoRows) {
			return nil, ErrNotFound("documents")
